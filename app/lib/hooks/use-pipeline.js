import { useEffect, useState } from "react";

/**
 * Hook to build a Transformers.js pipeline function.
 *
 * Similar to `pipeline()`, but runs inference in a separate
 * Web Worker thread and asynchronous logic is
 * abstracted for you.
 *
 * *Important:* `options` must be memoized (if passed),
 * otherwise the hook will continuously rebuild the pipeline.
 */
export function usePipeline(task, model, options) {
  const [worker, setWorker] = useState();
  const [pipe, setPipe] = useState();

  useEffect(() => {
    const { progress_callback, ...transferableOptions } = options ?? {};

    const worker = new Worker(
      new URL("../workers/pipeline.js", import.meta.url),
      {
        type: "module",
      }
    );

    const onMessageReceived = (e) => {
      const { type } = e.data;

      switch (type) {
        case "progress": {
          const { data } = e.data;
          if (progress_callback) {
            progress_callback(data);
          }
          break;
        }
        case "ready": {
          setWorker(worker);
          break;
        }
      }
    };

    worker.addEventListener("message", onMessageReceived);

    worker.postMessage({
      type: "init",
      args: [task, model, transferableOptions],
    });

    return () => {
      worker.removeEventListener("message", onMessageReceived);
      worker.terminate();

      setWorker(undefined);
    };
  }, [task, model, options]);

  useEffect(() => {
    if (!worker) {
      return;
    }

    // ID to sync return values between multiple ongoing pipe executions
    let currentId = 0;

    const callbacks = new Map();

    const onMessageReceived = (e) => {
      switch (e.data.type) {
        case "result":
          const { id, data: serializedData } = e.data;
          const { type, data, dims } = serializedData;
          const output = { type, data, dims }; // Mimicking Tensor object construction
          const callback = callbacks.get(id);

          if (!callback) {
            throw new Error(`Missing callback for pipe execution id: ${id}`);
          }

          callback(output);
          break;
      }
    };

    worker.addEventListener("message", onMessageReceived);

    const pipe = (...args) => {
      if (!worker) {
        throw new Error("Worker unavailable");
      }

      const id = currentId++;

      return new Promise((resolve) => {
        callbacks.set(id, resolve);
        worker.postMessage({ type: "run", id, args });
      });
    };

    setPipe(() => pipe);

    return () => {
      worker?.removeEventListener("message", onMessageReceived);
      setPipe(undefined);
    };
  }, [worker]);

  return pipe;
}
